package fr.bobinho.beconomy.api.scheduler;
...
public final class BScheduler {
    ...
    public synchronized int run(@Nonnull Runnable runnable) throws IllegalArgumentException {
        ...
        if (type == Type.SYNC) {
            if (getRepeatingDelay() != 0) {
                bukkitTaskId = Bukkit.getScheduler().runTaskTimer(BEconomyCore.getInstance(), runnable, getDelay(), getRepeatingDelay()).getTaskId();

            } else if (getDelay() != 0) {
                bukkitTaskId = Bukkit.getScheduler().runTaskLater(BEconomyCore.getInstance(), runnable, getDelay()).getTaskId();

            } else {
                bukkitTaskId = Bukkit.getScheduler().runTask(BEconomyCore.getInstance(), runnable).getTaskId();
            }

        } else {
            if (getRepeatingDelay() != 0) {
                bukkitTaskId = Bukkit.getScheduler().runTaskTimerAsynchronously(BEconomyCore.getInstance(), runnable, getDelay(), getRepeatingDelay()).getTaskId();
            } else if (getDelay() != 0) {
                bukkitTaskId = Bukkit.getScheduler().runTaskLaterAsynchronously(BEconomyCore.getInstance(), runnable, getDelay()).getTaskId();
            } else {
                bukkitTaskId = Bukkit.getScheduler().runTaskAsynchronously(BEconomyCore.getInstance(), runnable).getTaskId();
            }
        }
    ...
    public synchronized void run(@Nonnull Consumer<BukkitTask> task) throws IllegalArgumentException {
        ...
        if (type == Type.SYNC) {
            if (getRepeatingDelay() != 0) {
                Bukkit.getScheduler().runTaskTimer(BEconomyCore.getInstance(), task, getDelay(), getRepeatingDelay());
            } else if (getDelay() != 0) {
                Bukkit.getScheduler().runTaskLater(BEconomyCore.getInstance(), task, getDelay());
            } else {
                Bukkit.getScheduler().runTask(BEconomyCore.getInstance(), task);
            }

        } else {
            if (getRepeatingDelay() != 0) {
                Bukkit.getScheduler().runTaskTimerAsynchronously(BEconomyCore.getInstance(), task, getDelay(), getRepeatingDelay());
            } else if (getDelay() != 0) {
                Bukkit.getScheduler().runTaskLaterAsynchronously(BEconomyCore.getInstance(), task, getDelay());
            } else {
                Bukkit.getScheduler().runTaskAsynchronously(BEconomyCore.getInstance(), task);
            }
        }
    }
